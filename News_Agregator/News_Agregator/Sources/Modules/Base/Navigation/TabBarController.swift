...
extension TabBarController {
    ...
    private func configureAppearance() {
        ...
        let controllers : [BaseNavController] = Tabs.allCases.map { tab in
            ...
            let image1 = imageWithImg(with: image, scaledTo: CGSize(width: R.Sizes.icon, height: R.Sizes.icon))
    ...
    private func getController(for tab: Tabs) -> BaseViewController {
        ...
        case .favorites: return FavoritesViewController(viewModel: FavoritesViewModel())
        }
    }
}
...
extension TabBarController {
    private func imageWithImg(with image: UIImage?, scaledTo newSize: CGSize) -> UIImage? {
